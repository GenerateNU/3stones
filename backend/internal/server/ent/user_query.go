...
func (uq *UserQuery) loadRefreshToken(ctx context.Context, query *RefreshTokenQuery, nodes []*User, init func(*User), assign func(*User, *RefreshToken)) error {
	...
	for _, n := range neighbors {
		fk := n.refresh_token_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "refresh_token_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "refresh_token_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
