...
var (
	// In the future, salt should either be rand-generated or stored in an env secret,
	// but for an MVP, it is fine to keep the salt constant for now.
	HASH_SALT       = "keep_me_here"
	STANDARD_PARAMS = hashParams{
		memory:      64 * 1024,
		iterations:  3,
		parallelism: 2,
		saltLength:  16,
...
func Hash(password string) (encodedHash string, err error) {
	...
	hash := argon2.IDKey(
		[]byte(password),
		salt,
		STANDARD_PARAMS.iterations,
		STANDARD_PARAMS.memory,
		STANDARD_PARAMS.parallelism,
		STANDARD_PARAMS.keyLength)
