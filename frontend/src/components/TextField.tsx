...
const textFieldVariants = cva('textField', {
  variants: {
    ...
    size: {
      medium: ['flex', 'p-[8px_12px]', 'items-center', 'gap-2', 'self-stretch'],
      large: ['flex', 'p-[12px_16px]', 'items-center', 'gap-2', 'self-stretch'],
    },
    icon: {
      default: [],
      left: ['flex-row', 'items-center', 'pl-4'],
      right: ['flex-row', 'items-center', 'pr-4', 'justify-between'],
      prefix: ['flex-row', 'items-center', 'pl-4'],
      suffix: ['flex-row', 'items-center', 'pr-4', 'justify-between'],
      counter: ['flex-row', 'justify-between', 'items-center', 'pr-4'],
    },
  },
});
...
const textFieldTextVariants = cva('text', {
  variants: {
    intent: {
      default: ['body-regular', 'color-gray-600'],
      hover: ['body-regular', 'color-gray-600'],
      focus: ['body-regular', 'color-default-text'],
      error: ['body-regular', 'color-default-text'],
      filled: ['body-regular', 'color-default-text'],
      disabled: ['body-regular', 'color-gray-500'],
    },
  },
});

export interface TextFieldProps extends TextInputProps, VariantProps<typeof textFieldVariants> {
  ...
  return (
    <StyledView className='p-4 gap-0.5 items-start flex-col'>
      <StyledView className={textFieldVariants({ size, intent, icon })}>
        {icon === 'left' && iconRoute && <StyledImage source={iconRoute} className='mr-2' />}
        {icon === 'prefix' && prefix && (
