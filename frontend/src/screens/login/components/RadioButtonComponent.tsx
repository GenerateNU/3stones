...
const RadioButtonComponent = ({ label, selected, onPress }) => (
  ...
  >
    <StyledText className={`text-lg font-medium ${selected ? 'text-primary' : 'text-black'}`}>
      {label}
    </StyledText>
    <StyledView
      className={`h-6 w-6 rounded-full border-2 flex items-center justify-center ${selected ? 'border-primary bg-defaultPrimary' : 'border-gray-400'}`}
    >
