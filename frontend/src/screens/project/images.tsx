...
// Putting this here for now since we'll probably need navigation here later
    ...
    <StyledView className='flex flex-col h-full w-full'>
      ...
      >
        <StyledView className='flex flex-row w-full h-[72px] gap-x-4'>
          {project.images.map((item, ind) => (
            <TouchableOpacity
              onPress={() => {
                ref.current?.scrollTo({ count: ind - progress.value, animated: true });
              }}
            >
