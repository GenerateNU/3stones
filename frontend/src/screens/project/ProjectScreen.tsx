...
export default function ProjectScreen({ navigation }: ProjectScreenProps) {
  ...
  return (
    <StyledScrollView
      className='flex-1 bg-surfaceBG p-6 text-defaultText'
    ...
    >
      ...
      <StyledView className='flex flex-row justify-between'>
        ...
        <StyledView className='flex-row items-center gap-2'>
          ...
          <StyledText className='font-bodyBold'>Status:</StyledText>
          <StyledText className='font-body'>In Progress</StyledText>
        </StyledView>
      </StyledView>
      ...
      <StyledTouchableOpacity
        className='bg-defaultPrimary rounded-full flex flex-row justify-center items-center h-10 p-3'
        onPress={(evt) => {
          navigation.navigate('project-invest');
        }}
      >
        <Text style={{ color: 'white' }}>
          {' '}
          {/* For some reason tailwindclasses doesnt work here*/}
          Invest
        </Text>
      </StyledTouchableOpacity>
      <StyledTouchableOpacity
        className='border border-defaultPrimary rounded-full flex flex-row justify-center items-center h-10 mt-2 mb-6'
        onPress={(evt) => {
          navigation.navigate('project-updates');
        }}
      >
        <Text style={{ color: '#285852' }}>
          {' '}
          {/* For some reason tailwindclasses doesnt work here*/}
          Updates
        </Text>
      ...
      <StyledText className='font-body text-base leading-5'>{project.description}</StyledText>
      ...
      <StyledView className='flex flex-col gap-y-2'>
        {propertyDetails.map((val, idx, arr) => (
          <StyledView className='flex flex-row justify-between'>
            <StyledText className='font-bodyBold text-base'>{val[0]}</StyledText>
